 ...
 * @returns 
 */
export async function signUp(req: any, res: any) {console.log('aaaaa');
    try {
        logger.debug(`signUp - ${inspect(req.body)}`);
        ...
        if (!user) {
            ...
            logger.debug(`Create a new user - ${inspect(user)}`);
...
export async function logIn(req: any, res: any) {
    try {
        logger.debug(`logIn - ${inspect(req.body)}`);
        ...
        logger.debug(`logIn - ${inspect(user)}`);
...
console.log(user);
        ...
        const seatReservation: matchMaker.SeatReservation = await matchmakerHelper.matchMakeToRoom("lobby_room", req.body.roomId);
